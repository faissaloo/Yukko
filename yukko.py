...
errorRetrievingPage=asciiArtLoader.asciiImg(settings["theme folder"]+"errorRetrievingPage.txt")
boardListBg=asciiArtLoader.asciiImg(settings["theme folder"]+"boardListBg.txt")
emptyBoard=asciiArtLoader.asciiImg(settings["theme folder"]+"emptyBoard.txt")
logo=asciiArtLoader.asciiImg(settings["theme folder"]+"logo.txt")
attachments=asciiArtLoader.asciiImg(settings["theme folder"]+"attachmentsBg.txt")
...
def threadOverviewLength(thread,ypad,offset,maxlines,maxposts):
	...
	for i in thread.overview(3):
		total+=min(getPostHeight(i,ypad),ypad+maxlines+1) #Maxlines+1 because a message is appended to those that exceed it
	return total
	...
	while not (last_ch==27 or last_ch==ord("\n")):
		...
		if last_ch==curses.KEY_LEFT:
			...
			if cursor_pos>0:
				...
				if cursor_pos<scope_pos:
					scope_pos-=1
		elif (not (last_ch==27 or last_ch==ord("\n") or intOnly)) or ((not (last_ch==27 or last_ch==ord("\n"))) and intOnly and last_ch<=ord("9") and 
		last_ch>=ord("0")) and (maxlen==0 or len(string)<maxlen):
	...
	while True:
		...
		if currKey==curses.KEY_UP:
			...
			if page>0:
				...
				y=0
				page-=1
				scr.addstr(0,0,("Loading page "+str(page)+" of "+board).ljust(windowW),curses.A_REVERSE)
				scr.refresh()
				currBoard=nntp.board(board,page)
			...
			next_board=textBox(0,len("Board: "),board,windowW-len("Board: "),False)
...
def threadView(thread):
	...
	while True:
		...
		elif currKey==27 or currKey==curses.KEY_BACKSPACE or currKey==curses.KEY_LEFT:
			return
		scr.clear()
	...
	while response!=201:
		while captchaString=="":
			...
			if captchaString=="": #Regen new captcha if we have to
				...
				nntp.getCaptcha()
			scr.refresh()
		response=object.post(postDict['Name'],postDict['Subject'],postDict['Body'],captchaString)
		if response==200:
			scr.addstr(0,0,("CAPTCHA failed, please try again.").ljust(windowW),curses.A_REVERSE)
			scr.refresh()
			captchaString=""
			nntp.cleanupCaptcha()
			nntp.getCaptcha()
			...
			scr.addstr(0,0,("Gateway timeout, press any key to try again").ljust(windowW),curses.A_REVERSE)
			scr.refresh()
			scr.getch()
...
def viewAttachments(post):
	...
	while True:
		...
		if currKey==curses.KEY_DOWN:
			...
			if r==200:
				scr.addstr(0,0,("Saved to "+settings["download directory"]+selectedFile.fileName).ljust(windowW),curses.A_REVERSE)
			scr.refresh()
			scr.getch()
		elif currKey==27 or currKey==curses.KEY_BACKSPACE or currKey==curses.KEY_LEFT:
			return
